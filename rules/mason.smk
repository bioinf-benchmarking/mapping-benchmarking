...
def get_truth_vcf_command(wildcards, input, output):
    ...
    if individual_config["simulated"]:
        ...
        return f"mason_variator -ir {input.reference} -ov {tmp_output} {parameters} && cat {tmp_output} | python scripts/assign_random_genotypes_to_vcf.py | bgzip -c > {output.vcf}"
    else:
